      ...
      await new Promise<void>((resolve, reject) => {
        const interval = setInterval(async () => {
          try {
            ...
            if (status === 'done') { clearInterval(interval); resolve(); }
            else if (status === 'error') { clearInterval(interval); reject(new Error(error || 'Render failed')); }
          } catch (e) { console.warn('[poll] retrying...', e); }
        }, 3000);
      });
...
export default function App() {
  ...
  const handleSaveCustomPreset = (name: string) => {
    ...
    const next = { ...customPresets, [key]: { label: name.trim(), emoji: "⭐", fontName: style.fontName, fontSize: style.fontSize, primaryColor: style.primaryColor, outlineColor: style.outlineColor, bgOpacity: style.bgOpacity } };
  ...
  const resetAll = () => {
    ...
    if (originalSubtitles.length > 0) setSubtitles(originalSubtitles);
  };

  const stripEmphasisTags = (subs: Subtitle[]) => subs.map(sub => ({ ...sub, text: sub.text.replace(/\*\*/g, '') }));
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .sec-label{font-size:11px;color:var(--mut);font-weight:500;letter-spacing:.05em;text-transform:uppercase;margin-bottom:6px;display:flex;align-items:center;justify-content:space-between}
  .sec-label .val{color:var(--txt);font-family:'JetBrains Mono',monospace;font-size:10px;text-transform:none}
  .presets-wrap{display:flex;flex-wrap:wrap;gap:5px}
  .preset-pill{background:var(--s2);border:1.5px solid var(--brd);border-radius:20px;padding:5px 10px;font-size:12px;color:var(--txt);cursor:pointer;transition:all .2s;display:flex;align-items:center;gap:4px}
  .preset-pill:hover{border-color:var(--amb)}
  .preset-pill.on{border-color:var(--amb);background:var(--amd);color:var(--amb)}
  .preset-pill.custom.on{border-color:#cc99ff;background:rgba(204,153,255,0.15);color:#cc99ff}
  .preset-pill.save{border-style:dashed;color:var(--mut)}
  .preset-pill.save:hover{border-color:var(--grn);color:var(--grn)}
  .preset-pill.reset{color:var(--mut);border-style:dotted}
  .preset-pill.reset:hover{border-color:var(--red);color:var(--red)}
  .preset-del{margin-left:4px;color:var(--mut);font-size:14px;line-height:1}
  .preset-del:hover{color:var(--red)}
  .divider{height:1px;background:var(--brd);margin:8px 0}
  .sel{background:var(--s2);border:1.5px solid var(--brd);border-radius:var(--r);color:var(--txt);font-size:13px;padding:8px 10px;cursor:pointer;outline:none;width:100%}
  .sel:focus{border-color:var(--amb)}
  .size-row{display:flex;align-items:center;gap:6px}
  .size-btn{width:28px;height:28px;border-radius:6px;border:1.5px solid var(--brd);background:var(--s2);color:var(--txt);font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center}
  .size-btn:hover{border-color:var(--amb);color:var(--amb)}
  .rng{flex:1;-webkit-appearance:none;height:4px;border-radius:2px;background:var(--s2);outline:none}
  .rng::-webkit-slider-thumb{-webkit-appearance:none;width:14px;height:14px;border-radius:50%;background:var(--amb);cursor:pointer}
  .size-num{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--mut);min-width:22px;text-align:right}
  .color-row{display:flex;align-items:center;gap:8px;margin-bottom:6px}
  .cpick{-webkit-appearance:none;width:28px;height:28px;border:none;border-radius:6px;cursor:pointer;background:none;padding:0}
  .cpick::-webkit-color-swatch-wrapper{padding:0}
  .cpick::-webkit-color-swatch{border:1.5px solid var(--brd);border-radius:6px}
  .chex{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--mut);text-transform:uppercase}
  .chips{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px}
  .chip{width:22px;height:22px;border-radius:4px;cursor:pointer;transition:transform .15s}
  .chip:hover{transform:scale(1.15)}
  .chip.on{box-shadow:0 0 0 2px var(--amb)}
  .sub-list{flex:1;overflow-y:auto;padding:8px}
  .sub-item{display:flex;align-items:flex-start;gap:8px;padding:8px;border-radius:var(--r);transition:background .15s}
  .sub-item:hover{background:var(--s2)}
  .sub-item.hi{background:rgba(245,158,11,0.08)}
  .si-n{width:22px;text-align:right;font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--mut);padding-top:4px;flex-shrink:0}
  .si-body{flex:1;min-width:0;display:flex;flex-direction:column;gap:4px}
  .si-times{display:flex;align-items:center;gap:4px}
  .tc{width:72px;background:var(--s2);border:1.5px solid var(--brd);border-radius:5px;color:var(--txt);font-family:'JetBrains Mono',monospace;font-size:10px;padding:3px 5px;outline:none}
  .tc:focus{border-color:var(--amb)}
  .tc-arr{color:var(--mut);font-size:10px}
  .si-txt{width:100%;background:var(--s2);border:1.5px solid var(--brd);border-radius:var(--r);color:var(--txt);font-size:12px;padding:6px 8px;outline:none;resize:none;min-height:36px}
  .si-txt:focus{border-color:var(--amb)}
  .si-meta{display:flex;flex-direction:column;align-items:center;gap:4px;padding-top:4px}
  .cdot{width:6px;height:6px;border-radius:50%}
  .del-btn{background:none;border:none;color:var(--mut);font-size:16px;cursor:pointer;line-height:1;padding:2px}
  .del-btn:hover{color:var(--red)}
  .export-panel{width:100%;max-width:480px;padding:28px 20px}
  .export-panel h3{font-family:'Syne',sans-serif;font-weight:800;font-size:20px;margin-bottom:4px}
  .export-sub{color:var(--mut);font-size:13px;margin-bottom:16px}
  .export-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}
  .export-card{background:var(--s1);border:1.5px solid var(--brd);border-radius:var(--rl);padding:16px 12px;text-align:center;cursor:pointer;transition:all .2s;display:flex;flex-direction:column;align-items:center;gap:6px}
  .export-card:hover{border-color:var(--amb);background:var(--amd)}
  .export-card.accent{border-color:var(--amb);background:var(--amd)}
  .export-card.accent:hover{background:var(--amg)}
  .export-card.loading{opacity:.7;cursor:wait}
  .export-card.done{border-color:var(--grn);background:rgba(52,211,153,0.1)}
  .ei{font-size:22px}
  .el{font-size:13px;font-weight:500}
  .ed{font-size:10px;color:var(--mut)}
  .mt8{margin-top:8px}
`;
