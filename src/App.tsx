...
const DEFAULT_STYLE: SubStyle = {
  ...
  bgOpacity:0, preset:"impact", box:DEFAULT_BOX,
};
...
function SubtitleBox({text,style,onChange}:{text:string;style:SubStyle;onChange:(s:SubStyle)=>void}){
  ...
  useEffect(()=>{
    const h=(e:MouseEvent)=>{if(ref.current&&!ref.current.contains(e.target as Node))setSel(false);};
  ...
  const handles=[
    ...
    {k:"n",s:{top:-5,left:"50%" as any,transform:"translateX(-50%)",cursor:"n-resize"}},
    {k:"s",s:{bottom:-5,left:"50%" as any,transform:"translateX(-50%)",cursor:"s-resize"}},
    {k:"e",s:{right:-5,top:"50%" as any,transform:"translateY(-50%)",cursor:"e-resize"}},
    {k:"w",s:{left:-5,top:"50%" as any,transform:"translateY(-50%)",cursor:"w-resize"}},
  ];
  ...
  return(
    <div ref={ref}
      style={{
        ...
        border:sel?"2px solid #f59e0b":"1.5px dashed rgba(255,255,255,0.45)",
    ...
    >
      <span style={{
        ...
        maxWidth:"98%",wordBreak:"break-word",
        whiteSpace:"normal",display:"block",
        pointerEvents:"none",userSelect:"none",
      }}>{text||"Sample text"}</span>
      {sel&&handles.map(h=>(
        <div key={h.k}
          style={{position:"absolute",width:10,height:10,background:"#f59e0b",border:"1.5px solid #fff",borderRadius:2,zIndex:30,...h.s}}
          onPointerDown={e=>pd(e,h.k)}
          onPointerMove={pm}
          onPointerUp={pu}
        />
      ))}
      {sel&&<div style={{position:"absolute",top:-20,right:0,fontSize:9,color:"rgba(255,255,255,0.6)",background:"rgba(0,0,0,0.6)",padding:"2px 5px",borderRadius:3,pointerEvents:"none",whiteSpace:"nowrap"}}>click outside to deselect</div>}
    </div>
  );
}
      ...
      {tab==="style"&&(
        <div className="tab-body">
          ...
          <div className="color-row">
            <input type="color" value={style.primaryColor} onChange={e=>set({primaryColor:e.target.value})} className="cpick"/>
          ...
          <div className="color-row">
            <input type="color" value={style.outlineColor} onChange={e=>set({outlineColor:e.target.value})} className="cpick"/>
...
const CSS=`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* Right panel */
  .right-panel{width:300px;flex-shrink:0;border-left:1px solid var(--brd);display:flex;flex-direction:column;overflow:hidden;background:var(--s1)}
  ...
  .size-btn{background:var(--s2);border:1px solid var(--brd);border-radius:6px;color:var(--txt);font-size:16px;width:26px;height:26px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .15s;line-height:1}
  ...
  .rng{flex:1;accent-color:var(--amb);cursor:pointer;height:4px}
  ...
  @media(max-width:700px){.right-panel{width:260px}.editor-page{flex-direction:column}.vid-col{flex:none;height:50%}.right-panel{width:100%;border-left:none;border-top:1px solid var(--brd)}}
`;
